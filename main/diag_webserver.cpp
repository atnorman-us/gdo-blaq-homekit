...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    static const char *page =
        ...
        "input[type=\"number\"],input[type=\"file\"],input[type=\"password\"]{font-family:inherit;font-size:13px;border:1px solid var(--line);border-radius:6px;padding:6px 9px;background:var(--bg);color:var(--ink);}"
        "input[type=\"number\"]{width:64px;}"
        "input[type=\"password\"]{width:100%;box-sizing:border-box;margin-top:8px;}"
        "#pwPromptOverlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:1000;align-items:center;justify-content:center;}"
        "#pwPromptOverlay .card{width:280px;max-width:calc(100vw - 32px);}"
        "#pwPromptOverlay .btnrow{justify-content:flex-end;margin-top:12px;}"
        ...
        "</div>"
        "<div id=\"pwPromptOverlay\">"
        "  <div class=\"card\">"
        "    <div id=\"pwPromptLabel\">Enter the admin password:</div>"
        "    <input type=\"password\" id=\"pwPromptInput\" autocomplete=\"off\">"
        "    <div class=\"btnrow\">"
        "      <button onclick=\"pwPromptCancel()\">Cancel</button>"
        "      <button class=\"primary\" onclick=\"pwPromptSubmit()\">OK</button>"
        "    </div>"
        "  </div>"
        "</div>"
        "<div class=\"grid\">"
        ...
        "<script>"
        "let adminToken = '';"
        "let pwPromptResolve = null;"
        "function askPassword(label){"
        "  return new Promise(resolve=>{"
        "    pwPromptResolve = resolve;"
        "    document.getElementById('pwPromptLabel').textContent = label;"
        ...
        "  });"
        "}"
        "function pwPromptFinish(value){"
        "  document.getElementById('pwPromptOverlay').style.display = 'none';"
        "  const resolve = pwPromptResolve; pwPromptResolve = null;"
        "  if(resolve) resolve(value);"
        "}"
        "function pwPromptSubmit(){ pwPromptFinish(document.getElementById('pwPromptInput').value || ''); }"
        "function pwPromptCancel(){ pwPromptFinish(null); }"
        "document.addEventListener('DOMContentLoaded', ()=>{"
        "  document.getElementById('pwPromptInput').addEventListener('keydown', e=>{"
        "    if(e.key==='Enter') pwPromptSubmit();"
        "    if(e.key==='Escape') pwPromptCancel();"
        "  });"
        "});"
        "async function authHeaders(){"
        "  if(!adminToken) adminToken = await askPassword('Enter the admin password:') || '';"
        "  return {'X-GDO-Token':adminToken};"
        "}"
        "async function setupAdminPassword(){"
        "  const pw1 = await askPassword('Set an admin password for this device (min 8 characters):');"
        "  if(!pw1) return;"
        "  if(pw1.length < 8){ alert('Password must be at least 8 characters.'); return; }"
        "  if(pw1 !== await askPassword('Confirm the admin password:')){ alert('Passwords did not match.'); return; }"
        "  try{"
        ...
        "  try{"
        "    const r = await fetch('/restart', {method:'POST',headers:await authHeaders()});"
        ...
        "  try{"
        "    const headers = await authHeaders(); if(!adminToken) return;"
        ...
        "  }catch(e){}"
        "  try{"
        "    if(requestToken) await authHeaders();"
        ...
        "    wrap.style.display = 'none';"
        "  };"
        "  xhr.setRequestHeader('X-GDO-Token', (await authHeaders())['X-GDO-Token']);"
        "  xhr.send(file);"
        "}"
        ...
        "  try{"
        "    const r = await fetch('/firmware/confirm', {method:'POST',headers:await authHeaders()});"
        "    if(!r.ok){ if(r.status===401) adminToken=''; throw new Error(await r.text()); }"
        ...
        "  const statusEl = document.getElementById('fwStatus');"
        "  try{ const r=await fetch('/firmware/cancel', {method:'POST',headers:await authHeaders()}); if(!r.ok){if(r.status===401) adminToken=''; throw new Error(await r.text());} }catch(e){statusEl.textContent='Cancel failed: '+e.message; return;}"
        "  document.getElementById('fwConfirmRow').style.display = 'none';"
        "  document.getElementById('fwUploadRow').style.display = 'flex';"
        ...
        "  try{"
        "    const r = await fetch('/firmware/rollback', {method:'POST',headers:await authHeaders()});"
        "    if(r.ok){"
